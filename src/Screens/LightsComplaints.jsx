...
const LightsComplaints = () => {
  const [complaints, setComplaints] = useState([]);
  const [selectedComplaint, setSelectedComplaint] = useState(null);

  useEffect(() => {
    // Fetch all complaints from Firebase collection "StreetLight Complaints"
    const fetchComplaints = async () => {
      const querySnapshot = await getDocs(collection(db, 'StreetLight Complaints'));
  ...
  // Update complaint status in Firestore and show a message with container color change
  const updateComplaintStatus = async (complaintId, status) => {
    try {
      const complaintRef = doc(db, 'StreetLight Complaints', complaintId);
      ...
      // Show message based on the updated status
  ...
  // Get the appropriate background color based on the status
  const getStatusColor = (status) => {
    switch (status) {
      case 'Solved':
        return '#d0e8ff'; // Light Green
      case 'Pending':
        return '#fff5cc'; // Light Orange
      case 'Rejected':
    ...
    <div className="manage-complaints">
      ...
      <main className="main-content">
        <header className="main-header">
          <h1>StreetLight Complaints Confirmation</h1>
        </header>
      ...
      {selectedComplaint && (
        <div className="overlay">
          <div className="details-box form-container">
            ...
            <div className="form-group">
              <p><strong>Type:</strong> {selectedComplaint.type_of_waste || 'StreetLight Complaints'}</p>
            </div>
            <div className="form-group">
...
export default LightsComplaints;
